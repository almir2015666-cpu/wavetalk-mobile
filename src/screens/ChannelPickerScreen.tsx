...
export default function ChannelPickerScreen({ myName, onJoin, onBack }: Props) {
  ...
  useEffect(() => {
    ...
    fetchChannels();
    const timer = setInterval(fetchChannels, 5000);
  ...
  const handleJoin = (name: string) => {
    setJoining(name);
    setTimeout(() => onJoin(name), 120); // small delay for visual feedback
  };
      ...
      >
        ...
        {loading ? (
          ...
          channels.map(ch => {
            ...
            return (
              <TouchableOpacity
                key={ch.name}
                style={[s.chCard, isJoining && s.chCardActive]}
                onPress={() => handleJoin(ch.name)}
                activeOpacity={0.75}
              >
                <View style={s.chIcon}>
                ...
                <View style={s.chInfo}>
                  ...
                  <Text style={s.chMeta}>
                    {ch.online === 0
                      ...
                      : `${ch.online} ${ch.online === 1 ? 'pessoa' : 'pessoas'} online`}
                    {ch.talking ? ' · 🎙 transmitindo' : ''}
                  </Text>
                </View>
                ...
                <View style={s.chRight}>
                  ...
                  <Text style={s.arrow}>{isJoining ? '…' : '→'}</Text>
                </View>
              </TouchableOpacity>
            );
          })
        ...
        <View style={s.createCard}>
          <TextInput
            ...
            autoCapitalize="none"
            returnKeyType="go"
            onSubmitEditing={() => canCreate && handleJoin(newChannel.trim())}
            selectionColor={C.cyan}
          />

          <TouchableOpacity
            style={[s.createBtn, !canCreate && { opacity: 0.35 }]}
            onPress={() => canCreate && handleJoin(newChannel.trim())}
            disabled={!canCreate}
            activeOpacity={0.8}
          >
            {canCreate ? (
